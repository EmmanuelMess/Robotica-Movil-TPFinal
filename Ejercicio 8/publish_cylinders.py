...
class PublishCylindersServer(Node):
    ...
    def __init__(self):
        ...
        self.publisher = self.create_publisher(MarkerArray, "/visualization_marker/marker", 2)
        self.publisherPointCloud = self.create_publisher(PointCloud2, "/visualization_marker/point_cloud", 2)
        self.subscriber = self.create_subscription(LaserScan, '/scan', self.execute_callback, 10)
        self.subscriber2 = self.create_subscription(LaserScan, '/scan', self.execute_callback_pointcloud, 10)
    def execute_callback_pointcloud(self, scan):
        lp = lg.LaserProjection()
        pc2_msg = lp.projectLaser(scan)
        
        self.publisherPointCloud.publish(pc2_msg)
